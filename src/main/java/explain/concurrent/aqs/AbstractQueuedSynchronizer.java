...
package explain.concurrent.aqs;

import explain.concurrent.UnsafeInstance;
import explain.concurrent.aqs.rwl.ReadWriteLock;
...
public abstract class AbstractQueuedSynchronizer
     ...
     * @return node's predecessor, 入队成功后，传入节点的前驱节点
     */
    private Node enq(final Node node) {
        for (;;) {
            Node t = tail;
            if (t == null) { // Must initialize
                // 尾节点为空，说明是没有线程状态，头节点也必然为空
                if (compareAndSetHead(new Node()))
                    // 这里给头节点一个空节点占位，然后让尾节点也指向头节点
                    tail = head;
            } else {
                // 不为空，就单纯的没抢到，继续循环抢
                node.prev = t;
    ...
    private boolean parkAndCheckInterrupt() {
        ...
        // 查看是否中断唤醒，如果是，这里会消耗掉中断标识，不过后面可能会补上
     ...
     *
     * 重头戏，这时的线程已经在队列里了，在这里循环两次会进入阻塞状态
     *
     * @param node the node
    ...
    final boolean acquireQueued(final Node node, int arg) {
        ...
        try {
            ...
            for (;;) {
                // 这里p是node节点的前一个，就是之前的tail节点
                final Node p = node.predecessor();
                // 上一个节点是头节点，说明随时可能轮到当前节点，尝试获取锁一次
                ...
                // 判断是否可以park，里面会把前面取消状态的过掉,
                // 如果前一个节点状态是0会把它置为SINGAL，这样下一圈进来时就可以满足条件
                if (shouldParkAfterFailedAcquire(p, node)) {
                    // 一开始新建的节点状态是0
                    // 前一个节点状态必须是SIGNAL才会进来，当前才可以park，
                    // 下面的方法会park住，如果被唤醒了，说明可以尝试一下获取锁
    ...
    public final void acquire(int arg) {
        ...
        // 下面就是从等待队列中获取锁，就是排队等，会阻塞住线程，返回的是线程是否已中断
        if (this.acquireQueued(node, arg)) {
            // 到这里说明是中断唤醒，前面把中断标识消耗掉了，这里再加上
            selfInterrupt();
        }
    }
    ...
    public class ConditionObject implements Condition, java.io.Serializable {
        ...
        public final void await() throws InterruptedException {
            ...
            // 这里把当前线程搞个新节点加入到条件等待队列
            Node node = addConditionWaiter();
            // 这里会释放锁，如果释放失败了会被标记为取消状态，并抛异常
            int savedState = fullyRelease(node);
            int interruptMode = 0;
            while (!isOnSyncQueue(node)) {
                LockSupport.park(this);
